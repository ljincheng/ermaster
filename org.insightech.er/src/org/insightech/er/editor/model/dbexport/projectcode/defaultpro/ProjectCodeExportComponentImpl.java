 ...
 *
 */
public class ProjectCodeExportComponentImpl extends ProjectCodeTemplate{
	...
	static {
		try {
			TEMPLATE = ProjectCodeCommon.loadResource("@componentImpl");
			TEMPLATE_PRIMARYKEY = ProjectCodeCommon.loadResource("@componentImpl_primarykey");
	...
	public ProjectCodeExportComponentImpl(ProjectCodeSetting setting, ExportJavaSetting exportJavaSetting, ERDiagram diagram) {
...
//		this.importClasseNames.add(getClassName(table));
	...
	public String getOutPath(ERTable table) {
		...
		return folderPath+this.getClassName(table)+"ComponentImpl.java";
	}
	...
	private String replacePrimarykeyInfo(String content, ERTable table,String classDescription) {
		...
		if(table.getPrimaryKeySize()>0)
		{
			...
			content = content.replace("@componentImpl_primarykey", TEMPLATE_PRIMARYKEY);
...
//		content = this.replaceClassInfo(content, classDescription,ProjectCodeCommon.getCamelCaseName(table),this.exportJavaSetting.getClassNameSuffix());
			...
			content = content.replace("@componentImpl_primarykey", "");
		}
		return content;
	}
	...
	public String getNamespace()
	{
		...
		String javaFolder=this.projectCodeSetting.getComponentImplPath();

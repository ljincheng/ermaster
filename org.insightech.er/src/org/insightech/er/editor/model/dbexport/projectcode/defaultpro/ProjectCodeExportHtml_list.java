 ...
 *
 */
public class ProjectCodeExportHtml_list extends ProjectCodeTemplate{

	private static final String TEMPLATE;

	static {
		try {
			TEMPLATE = ProjectCodeCommon.loadResource("@html_list");
		} catch (IOException e) {
			e.printStackTrace();
			throw new ExceptionInInitializerError(e);
		}
	}
	public ProjectCodeExportHtml_list(ProjectCodeSetting setting, ExportJavaSetting exportJavaSetting,
			ERDiagram diagram) {
...
//		content = this.replaceImportInfo(content);
	...
	public String getOutPath(ERTable table) {
		...
		return folderPath+ProjectCodeCommon.getCamelCaseName(this.getClassName(table),false,false)+"List.jsp";
	}

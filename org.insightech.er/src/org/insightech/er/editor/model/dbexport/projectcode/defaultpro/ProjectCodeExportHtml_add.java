...
import org.insightech.er.util.Check;
	 ...
	 *
	 */
	public class ProjectCodeExportHtml_add extends ProjectCodeTemplate{

		private static final String TEMPLATE;

		static {
			try {
				TEMPLATE = ProjectCodeCommon.loadResource("@html_add");
			} catch (IOException e) {
				e.printStackTrace();
				throw new ExceptionInInitializerError(e);
			}
		}
		public ProjectCodeExportHtml_add(ProjectCodeSetting setting, ExportJavaSetting exportJavaSetting,
				ERDiagram diagram) {
		...
		public String generateContent(ERTable table) throws IOException {
			...
			content=this.replaceAddFormHtml(content, table, ProjectCodeCommon.getCamelCaseName(table));
...
//			content = this.replaceImportInfo(content);
		...
		public String getOutPath(ERTable table) {
			...
			return folderPath+"add"+ProjectCodeCommon.getCamelCaseName(table)+".jsp";
		}
